        ...
        while (finding) {
            ...
            for (const seg of remaining) {
                ...
                if (distStart < EPSILON) {
                    for (let k = 1; k < seg.points.length; k++) currentLoop.push(seg.points[k]);
                    seg.used = true;
                    finding = true;
                    break;
                } else if (distEnd < EPSILON) {
                    const reversed = [...seg.points].reverse();
                    for (let k = 1; k < reversed.length; k++) currentLoop.push(reversed[k]);
                    seg.used = true;
                    finding = true;
                    break;
                }
...
const getPartGeometry = (part: PartData, placed: PlacedPartData): PartGeometry => {
    const rawSegments: GeometrySegment[] = [];
    const CURVE_SEGMENTS = 16; 
    
    // 1. Extração dos Segmentos (Igual ao anterior)
    const traverse = (entities: EntityData[], offsetX: number, offsetY: number) => {
        entities.forEach((ent: EntityData) => {
            if (ent.type === 'INSERT' && ent.name && part.blocks) {
                const block = part.blocks[ent.name];
                if (block && block.entities) {
                    traverse(block.entities, offsetX + (ent.position?.x || 0), offsetY + (ent.position?.y || 0));
                }
                return;
            }
            ...
            if (ent.type === 'LINE' || ent.type === 'LWPOLYLINE' || ent.type === 'POLYLINE') {
                if (ent.vertices && ent.vertices.length > 0) {
                    for (let i = 0; i < ent.vertices.length; i++) {
                        ...
                        if (v1.bulge && v1.bulge !== 0) {
                            ...
                            const segments = Math.max(2, Math.floor(CURVE_SEGMENTS * (Math.abs(sweepAngle) / (2 * Math.PI))));

                            for (let j = 1; j < segments; j++) {
                                const theta = startAngle + (j / segments) * sweepAngle;
                                segPoints.push({ x: cx + radius * Math.cos(theta), y: cy + radius * Math.sin(theta) });
                            }
                        }
                    }
                }
                 ...
                 if (ent.center && ent.radius !== undefined) {
                     ...
                     for (let i = 0; i <= CURVE_SEGMENTS; i++) { 
                         ...
                         segPoints.push({ x: cx + r * Math.cos(theta), y: cy + r * Math.sin(theta) });
                     }
                 }
            }
                ...
                if (ent.center && ent.radius !== undefined) {
                     ...
                     const arcSegments = Math.max(2, Math.floor(CURVE_SEGMENTS * (sweep / (2 * Math.PI))));
...
self.onmessage = (e: MessageEvent<WorkerData>) => {
    ...
    placedParts.forEach(placed => {
        ...
        if (geom.bounds.minX >= minSafeX && geom.bounds.maxX <= maxSafeX && 
            geom.bounds.minY >= minSafeY && geom.bounds.maxY <= maxSafeY) {
            return; 
        }
    ...
    for (let i = 0; i < placedParts.length; i++) {
        for (let j = i + 1; j < placedParts.length; j++) {
            ...
            if (geomA.bounds.maxX < geomB.bounds.minX || geomA.bounds.minX > geomB.bounds.maxX || 
                geomA.bounds.maxY < geomB.bounds.minY || geomA.bounds.minY > geomB.bounds.maxY) {
                continue; 
            }
