...
import type { LabelStateMap, LabelConfig } from '../components/labels/LabelTypes';
...
export const useLabelManager = (parts: ImportedPart[]) => {
    ...
    const toggleGlobal = useCallback((type: 'white' | 'pink', forceValue?: boolean) => {
        setLabelStates(prev => {
            const next = { ...prev };
    ...
    const togglePartFlag = useCallback((partId: string, type: 'white' | 'pink') => {
        setLabelStates(prev => {
    ...
    const updateLabelConfig = useCallback((partId: string, type: 'white' | 'pink', changes: Partial<LabelConfig>) => {
        setLabelStates(prev => {
            const currentPartState = prev[partId];
            if (!currentPartState) return prev;
