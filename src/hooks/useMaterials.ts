...
export const useMaterials = (isTrial: boolean, token?: string) => {
  ...
  const fetchFromAPI = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/api/materiais?t=${Date.now()}`,
        {
          headers: {
  ...
  const addOptimisticMaterial = useCallback((newMaterial: IMaterial) => {
    setDbMaterials((prev) => {
      ...
      const updated = [...prev, newMaterial].sort((a, b) =>
  ...
  const addOptimisticEspessura = useCallback(
    (materialId: number, newEspessura: IEspessura) => {
      setDbMaterials((prev) =>
        prev.map((mat) => {
          if (mat.id === materialId) {
            // Cria uma nova versão do material com a nova espessura adicionada
            const novasEspessuras = [...mat.espessuras, newEspessura].sort(
              (a, b) => Number(a.milimetros) - Number(b.milimetros),
            ); // Mantém ordenado por espessura
  ...
  const getThicknessOptions = useCallback(
  ...
  const getMaterialIdByName = useCallback(
    (name: string) => {
