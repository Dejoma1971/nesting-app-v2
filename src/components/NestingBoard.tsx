...
const renderEntityFunction = (entity: any, index: number, blocks: any, scale = 1, color: string = "currentColor"): React.ReactNode => {
  switch (entity.type) {
    case "INSERT": {
    ...
    case "POLYLINE": {
      ...
      for (let i = 0; i < entity.vertices.length; i++) {
        ...
        if (v1.bulge && v1.bulge !== 0) {
          ...
          const rx = radius * scale; const ry = radius * scale;
          const largeArc = Math.abs(v1.bulge) > 1 ? 1 : 0; const sweep = v1.bulge > 0 ? 1 : 0;
          d += ` A ${rx} ${ry} 0 ${largeArc} ${sweep} ${v2.x * scale} ${v2.y * scale}`;
    ...
    case "ARC": {
      ...
      const x1 = center.x * scale + r * Math.cos(startAngle); const y1 = center.y * scale + r * Math.sin(startAngle);
      const x2 = center.x * scale + r * Math.cos(endAngle); const y2 = center.y * scale + r * Math.sin(endAngle);
...
export const NestingBoard: React.FC<NestingBoardProps> = ({
  ...
  const [direction, setDirection] = useState<"auto" | "vertical" | "horizontal">("horizontal");
  ...
  const [quantities, setQuantities] = useState<{ [key: string]: number }>(() => {
    ...
    initialParts.forEach((p) => { initialQ[p.id] = p.quantity || 1; });
    return initialQ;
  });
  ...
  const { isBinSaved, markBinAsSaved, resetAllSaveStatus } = useNestingSaveStatus(nestingResult);
      ...
      const addLabelVector = (config: LabelConfig, color: string, type: "white" | "pink") => {
        if (config.active && finalText) {
          ...
          const rotatedLines = vectorLines.map((line: any) => {
            ...
            return {
              ...line,
              vertices: [rotatePoint(line.vertices[0].x, line.vertices[0].y), rotatePoint(line.vertices[1].x, line.vertices[1].y)],
            };
          });
          const taggedLines = rotatedLines.map((line: any) => ({ ...line, isLabel: true, labelType: type, partId: part.id, color: color }));
  ...
  useEffect(() => {
    setQuantities((prev) => {
      ...
      if (missingParts.length > 0) {
        const newQ = { ...prev };
        missingParts.forEach((p) => { newQ[p.id] = p.quantity || 1; });
        return newQ;
      }
      return prev;
    });
  ...
  const handleReturnToBank = useCallback((uuidsToRemove: string[]) => {
      ...
      setNestingResult((prev) => prev.filter(p => !uuidsToRemove.includes(p.uuid)));
      setSelectedPartIds([]);
      if (partIdToScroll) {
          setTimeout(() => {
  ...
  const handleSaveClick = async () => {
    ...
    await handleProductionDownload(nestingResult, currentBinIndex, displayedParts);
  ...
  const handleCalculate = useCallback(() => {
    ...
    if (nestingResult.length > 0) {
      if (!window.confirm("O cálculo automático irá REORGANIZAR TODA A MESA... Deseja continuar?")) return;
    }
    setIsComputing(true);
    resetNestingResult([]);
  ...
  }, [displayedParts, quantities, gap, margin, binSize, strategy, iterations, rotationStep, direction, resetNestingResult, nestingResult.length, resetAllSaveStatus]);

  const handleClearTable = useCallback(() => {
    if (window.confirm("ATENÇÃO: Isso limpará a mesa de corte E O BANCO DE PEÇAS. Deseja reiniciar?")) {
      resetNestingResult([]);
      setParts([]);
      setFailedCount(0);
      setTotalBins(1);
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        if (searchMode === "replace") {
          if (nestingResult.length > 0 && !window.confirm("Isso limpará o arranjo atual. Continuar?")) {
            setIsSearching(false);
            return;
          }
          setParts(dbParts);
  ...
  const handleContextRotate = useCallback((angle: number) => {
      ...
      setNestingResult((prev) => prev.map((p) => selectedPartIds.includes(p.uuid) ? { ...p, rotation: (p.rotation + angle) % 360 } : p));
  }, [selectedPartIds, setNestingResult]);

  const handleContextMove = useCallback((dx: number, dy: number) => {
      if (selectedPartIds.length === 0) return;
      setNestingResult((prev) => prev.map((p) => selectedPartIds.includes(p.uuid) ? { ...p, x: p.x + dx, y: p.y - dy } : p));
  }, [selectedPartIds, setNestingResult]);
  ...
  const totalPlacedCounts = useMemo(() => {
    ...
    nestingResult.forEach((p) => { counts[p.partId] = (counts[p.partId] || 0) + 1; });
    return counts;
  }, [nestingResult]);
  ...
  const sortedParts = useMemo(() => {
    const sorted = [...displayedParts].sort((a, b) => {
      const aSel = selectedPartIds.includes(a.id); const bSel = selectedPartIds.includes(b.id);
      if (aSel && !bSel) return -1; if (!aSel && bSel) return 1;
      const aOnBoard = currentBinPartIds.has(a.id); const bOnBoard = currentBinPartIds.has(b.id);
      if (aOnBoard && !bOnBoard) return -1; if (!aOnBoard && bOnBoard) return 1;
      return 0;
    });
    return sorted;
  ...
  const toolbarStyle: React.CSSProperties = { padding: "10px 20px", borderBottom: `1px solid ${theme.border}`, display: "flex", gap: "15px", alignItems: "center", backgroundColor: theme.panelBg, flexWrap: "nowrap", overflowX: "auto" };
  ...
  const renderProgressBar = (produced: number, total: number, color: string) => {
    ...
    return (
      <div style={{ width: "100%", background: theme.border, height: "6px", borderRadius: "3px", marginTop: "5px", overflow: "hidden" }}>
        <div style={{ width: `${pct}%`, background: pct >= 100 ? "#28a745" : color, height: "100%" }}></div>
      </div>
    );
  };
  ...
  return (
    <div style={containerStyle}>
      {isSearchModalOpen && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.6)", zIndex: 9999, display: "flex", justifyContent: "center", alignItems: "center" }} onClick={() => setIsSearchModalOpen(false)}>
          <div style={{ backgroundColor: theme.panelBg, padding: "25px", borderRadius: "8px", width: "350px", border: `1px solid ${theme.border}`, boxShadow: "0 4px 15px rgba(0,0,0,0.3)" }} onClick={(e) => e.stopPropagation()}>
            <h3 style={{ marginTop: 0, color: theme.text }}>🔍 Buscar Pedido(s)</h3>
            <p style={{ fontSize: "13px", color: theme.label }}>Separe múltiplos pedidos por vírgula.</p>
            <div style={{ marginBottom: "15px", display: "flex", flexDirection: "column", gap: "10px", background: theme.inputBg, padding: "10px", borderRadius: "4px" }}>
              <span style={{ fontSize: "11px", fontWeight: "bold", opacity: 0.7 }}>MODO DE IMPORTAÇÃO:</span>
              <div style={{ display: "flex", gap: "15px" }}>
                <label style={{ display: "flex", alignItems: "center", cursor: "pointer", fontSize: "13px", color: theme.text }}>
                  <input type="radio" name="searchMode" checked={searchMode === "replace"} onChange={() => setSearchMode("replace")} style={{ marginRight: "5px" }} /> Nova Mesa (Limpar)
                </label>
                <label style={{ display: "flex", alignItems: "center", cursor: "pointer", fontSize: "13px", fontWeight: "bold", color: "#28a745" }}>
                  <input type="radio" name="searchMode" checked={searchMode === "append"} onChange={() => setSearchMode("append")} style={{ marginRight: "5px" }} /> Adicionar (Mix)
                </label>
              </div>
            </div>
            <input autoFocus value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleDBSearch()} placeholder="Ex: 35905, 35906" style={{ width: "100%", padding: "10px", marginTop: "5px", marginBottom: "20px", background: theme.inputBg, color: theme.text, border: `1px solid ${theme.border}`, borderRadius: "4px", boxSizing: "border-box" }} />
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px" }}>
              <button onClick={() => setIsSearchModalOpen(false)} style={{ padding: "8px 15px", background: "transparent", border: `1px solid ${theme.border}`, color: theme.text, borderRadius: "4px", cursor: "pointer" }}>Cancelar</button>
              <button onClick={handleDBSearch} disabled={isSearching} style={{ padding: "8px 15px", background: "#28a745", border: "none", color: "white", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }}>{isSearching ? "Buscando..." : "Buscar"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {contextMenu && contextMenu.visible && selectedPartIds.length > 0 && (
        <ContextControl x={contextMenu.x} y={contextMenu.y} onClose={() => setContextMenu(null)} onMove={handleContextMove} onRotate={handleContextRotate} />
      )}
      ...
      <div style={topBarStyle}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <button
            ...
            disabled={nestingResult.length === 0 || isSaving || isCurrentSheetSaved}
            style={{
              ...
              border: "none",
              padding: "6px 12px",
              cursor: (nestingResult.length === 0 || isSaving || isCurrentSheetSaved) ? "not-allowed" : "pointer",
              borderRadius: "4px",
              opacity: (nestingResult.length === 0 || isSaving || isCurrentSheetSaved) ? 0.6 : 1,
              fontSize: "13px",
              display: "flex",
              alignItems: "center",
              gap: "5px",
          ...
          >
            {isSaving ? "⏳ Salvando..." : isCurrentSheetSaved ? "✅ Chapa Salva" : "💾 Salvar DXF"}
          </button>

          <button onClick={handleClearTable} title="Reiniciar Página" style={{ background: "transparent", color: "#dc3545", border: `1px solid #dc3545`, padding: "5px 10px", borderRadius: "4px", cursor: "pointer", fontWeight: "bold", fontSize: "13px" }}>🗑️</button>
      ...
      <div style={toolbarStyle}>
        ...
        <label style={{ fontSize: "12px", display: "flex", alignItems: "center", cursor: "pointer", userSelect: "none" }}>
          <input type="checkbox" checked={showDebug} onChange={(e) => setShowDebug(e.target.checked)} style={{ marginRight: "5px" }} /> Ver Box
        </label>
        <button onClick={handleCalculate} disabled={isComputing} style={{ marginLeft: "auto", background: isComputing ? "#666" : "#28a745", color: "white", border: "none", padding: "8px 15px", cursor: isComputing ? "wait" : "pointer", borderRadius: "4px", fontWeight: "bold", fontSize: "13px", whiteSpace: "nowrap", boxShadow: "0 2px 4px rgba(0,0,0,0.2)" }}>
          {isComputing ? "..." : "▶ Calcular Nesting"}
        </button>
      </div>

      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        <div style={{ flex: 2, position: "relative", background: theme.canvasBg, display: "flex", flexDirection: "column", overflow: "hidden" }} onMouseDown={() => setContextMenu(null)} onDrop={handleCanvasDrop} onDragOver={handleCanvasDragOver}>
          ...
          {totalBins > 1 && (
            <div
              style={{
                ...
                padding: "5px 15px", borderRadius: "20px", boxShadow: "0 2px 5px rgba(0,0,0,0.2)",
                transition: "all 0.3s ease"
              }}
            >
              <button onClick={() => setCurrentBinIndex(Math.max(0, currentBinIndex - 1))} disabled={currentBinIndex === 0} style={{ cursor: "pointer", border: "none", background: "transparent", fontWeight: "bold", color: "inherit" }}>◀</button>
              <span style={{ fontWeight: "bold", fontSize: "13px" }}>Chapa {currentBinIndex + 1} de {totalBins}</span>
              <button onClick={() => setCurrentBinIndex(Math.min(totalBins - 1, currentBinIndex + 1))} disabled={currentBinIndex === totalBins - 1} style={{ cursor: "pointer", border: "none", background: "transparent", fontWeight: "bold", color: "inherit" }}>▶</button>
            </div>
          )}
            ...
            {activeTab === "grid" && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: "15px", alignContent: "start" }}>
                {sortedParts.map((part) => {
                  ...
                  if (isSelected) { mainBorderColor = "#007bff"; mainBorderWidth = "2px"; }
                  else if (isOnCurrentSheet) { mainBorderColor = "#28a745"; mainBorderWidth = "3px"; }

                  const cardBorderStyle = { borderLeft: `5px solid ${orderColor}`, borderTop: `${mainBorderWidth} solid ${mainBorderColor}`, borderRight: `${mainBorderWidth} solid ${mainBorderColor}`, borderBottom: `${mainBorderWidth} solid ${mainBorderColor}` };
                  ...
                  const originalW = box.width || 100; const originalH = box.height || 100;
                  const isTall = originalH > originalW; const p = Math.max(originalW, originalH) * 0.1;
                  let finalViewBox = ""; let contentTransform = "";

                  if (isTall) {
                    const cx = (box.minX + box.maxX) / 2; const cy = (box.minY + box.maxY) / 2;
                    contentTransform = `rotate(-90, ${cx}, ${cy})`;
                    const cameraW = originalH + p * 2; const cameraH = originalW + p * 2;
                    const cameraX = cx - cameraW / 2; const cameraY = cy - cameraH / 2;
                    finalViewBox = `${cameraX} ${cameraY} ${cameraW} ${cameraH}`;
                  } else { finalViewBox = `${box.minX - p} ${box.minY - p} ${originalW + p * 2} ${originalH + p * 2}`; }

                  return (
                    <div key={part.id} ref={(el) => { thumbnailRefs.current[part.id] = el; }} style={{ display: "flex", flexDirection: "column", alignItems: "center", position: "relative", opacity: 1, cursor: cursorStyle }} onContextMenu={(e) => handleThumbnailContextMenu(e, part.id)} draggable={canDrag} onDragStart={(e) => canDrag && handleDragStart(e, part)}>
                      <ThumbnailFlags partId={part.id} labelState={labelStates} onTogglePartFlag={togglePartFlag} />
                      <div style={{ width: "100%", aspectRatio: "1/1", background: theme.cardBg, borderRadius: "8px", marginBottom: "5px", padding: "10px", boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", boxShadow: isSelected ? "0 0 8px rgba(0,123,255,0.6)" : "none", transition: "all 0.2s ease", ...cardBorderStyle }}>
                        <svg viewBox={finalViewBox} style={{ width: "100%", height: "100%", overflow: "visible", color: theme.text, opacity: isDoneVisual ? 0.8 : 1, transition: "opacity 0.3s ease" }} transform="scale(1, -1)" preserveAspectRatio="xMidYMid meet">
                          <g transform={contentTransform}>
                            {part.entities.map((ent, i) => renderEntityFunction(ent, i, part.blocks, 1, isDoneVisual ? theme.border : theme.text))}
                          </g>
                        </svg>
                      </div>
                      <div style={{ width: "100%", display: "flex", flexDirection: "column", fontSize: "12px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                          <div style={{ display: "flex", flexDirection: "column", width: "65%" }}>
                            <span title={part.name} style={{ fontWeight: "bold", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{part.name}</span>
                            <span style={{ fontSize: "10px", fontWeight: "bold", color: orderColor }}>Ped: {part.pedido || "-"}</span>
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", background: isDoneVisual ? "rgba(40, 167, 69, 0.1)" : theme.hoverRow, padding: "2px 4px", borderRadius: "4px", border: `1px solid ${isDoneVisual ? "#28a745" : "transparent"}` }}>
                            <span style={{ fontSize: "10px", fontWeight: "bold", color: isDoneVisual ? "#28a745" : theme.text }}>{totalVisual}/{qty}</span>
                          </div>
                        </div>
                        {renderProgressBar(totalVisual, qty, orderColor)}
                        <div style={{ fontSize: "10px", color: isDoneVisual ? "#28a745" : theme.label, display: "flex", justifyContent: "space-between" }}>
                          <span>{isDoneVisual ? "✅ CONCLUÍDO" : "Em Produção"}</span>
                          <span style={{ fontWeight: "bold" }}>Falta: {remainingVisual}</span>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div style={{ overflowX: "auto", transform: "rotateX(180deg)", borderBottom: `1px solid ${theme.border}` }}>
                <table style={{ width: "100%", borderCollapse: "collapse", borderSpacing: 0, minWidth: "600px", transform: "rotateX(180deg)" }}>
                  <thead style={{ background: theme.panelBg }}>
                    <tr>
                      <th style={thStyle}>#</th><th style={thStyle}>Peça</th><th style={thStyle}>Pedido</th><th style={thStyle}>Mat/Esp</th><th style={thStyle}>Dimensões</th><th style={thStyle}>Área</th><th style={thStyle}>Meta</th><th style={thStyle}>Status Produção</th>
                    </tr>
                  </thead>
                  <tbody>
                    {sortedParts.map((part, index) => {
                      ...
                      const isDoneVisual = remainingVisual === 0;
                      const isOnCurrentSheet = currentBinPartIds.has(part.id);
                      const rowBg = isOnCurrentSheet ? "rgba(40, 167, 69, 0.05)" : isDoneVisual ? "rgba(40, 167, 69, 0.1)" : "transparent";

                      return (
                        <tr key={part.id} style={{ borderBottom: `1px solid ${theme.border}`, background: rowBg }}>
                          <td style={{ ...tdStyle, borderLeft: `4px solid ${orderColor}` }}>{index + 1}</td>
                          <td style={{ ...tdStyle, fontWeight: "bold" }} title={part.name}>{part.name}</td>
                          <td style={{ ...tdStyle, color: orderColor, fontWeight: "bold" }}>{part.pedido || "-"}</td>
                          ...
                          <td style={{ ...tdStyle, textAlign: "center" }}><span style={{ fontWeight: "bold", fontSize: "13px", color: theme.text }}>{qty}</span></td>
                          <td style={{ ...tdStyle, textAlign: "center" }}>
                            <div style={{ padding: "4px 8px", borderRadius: "4px", background: isDoneVisual ? "rgba(40, 167, 69, 0.1)" : theme.hoverRow, border: `1px solid ${isDoneVisual ? "#28a745" : theme.border}`, color: isDoneVisual ? "#28a745" : theme.text, fontWeight: "bold", fontSize: "12px", display: "inline-block", minWidth: "60px" }}>
                              {totalVisual} <span style={{ fontSize: "10px", fontWeight: "normal", opacity: 0.7 }}>de {qty}</span>
                            </div>
                          </td>
                          <td style={{ ...tdStyle, width: "120px" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "10px", marginBottom: "2px" }}><span style={{ fontWeight: "bold" }}>Saldo:</span><span>{remainingVisual === 0 ? "✅" : `-${remainingVisual}`}</span></div>
                            {renderProgressBar(totalVisual, qty, orderColor)}
                          </td>
                        </tr>
                      );
