...
export const NestingBoard: React.FC<NestingBoardProps> = ({ parts }) => {
  ...
  const formatArea = (mm2: number) =>
    mm2 > 100000 ? (mm2 / 1000000).toFixed(3) + " m²" : mm2.toFixed(0) + " mm²";
        ...
        return (
          ...
          >
            {block.entities.map((s: any, i: number) =>
              renderEntity(s, i, blocks, 1, color)
            )}
          </g>
        );
      }
  ...
  return (
    ...
    >
        ...
        {totalBins > 1 && (
          ...
          >
            ...
            <span style={{ fontWeight: "bold", fontSize: "13px" }}>
              {currentBinIndex + 1} / {totalBins}
            </span>
            <button
              onClick={() =>
                setCurrentBinIndex(Math.min(totalBins - 1, currentBinIndex + 1))
              }
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        {/* --- ÁREA DA CHAPA COM ZOOM E FUNDO TRANSPARENTE --- */}
        <div
          ref={svgContainerRef}
          style={{
            flex: 2,
            position: "relative",
            background: "transparent", // <--- CORREÇÃO: Fundo transparente para respeitar o tema
            display: "flex",
            flexDirection: "column",
        ...
        >
          ...
          >
            {/* Botões com fundo semitransparente para funcionarem no escuro e no claro */}
            <button
                ...
                <g transform={cncTransform}>
                  ...
                  {nestingResult
                    ...
                    .map((placed, i) => {
                      ...
                      if (!part) return null;
                      const rawBox = getRawBoundingBox(
                        part.entities,
                        part.blocks
                      );
                      ...
                      const occupiedW =
                        placed.rotation === 90 ? part.height : part.width;
                      const occupiedH =
                        placed.rotation === 90 ? part.width : part.height;

                      const centerMove = `translate(${-centerX}, ${-centerY})`;
                      const rotation = `rotate(${placed.rotation})`;
                      const finalMove = `translate(${
                        placed.x + occupiedW / 2
                      ...
                      return (
                        <g key={i}>
                          ...
                          <g
                            transform={`${finalMove} ${rotation} ${centerMove}`}
                          >
                            {part.entities.map((ent, j) =>
                              renderEntity(ent, j, part.blocks, 1, "#007bff")
                            )}
                          </g>
                        </g>
                      );
          ...
          >
            <span style={{ opacity: 0.6, fontSize: "12px" }}>
              {nestingResult.length > 0
                ? `Total de Peças: ${nestingResult.length}`
            ...
            {failedCount > 0 && (
              ...
              >
                ⚠️ {failedCount} PEÇAS NÃO COUBERAM
              </span>
            )}
          </div>
        </div>
        ...
        >
          {/* O restante do painel lateral segue a cor 'inherit', então já está correto */}
          <div
            style={{
              display: "flex",
              borderBottom: "1px solid #444",
