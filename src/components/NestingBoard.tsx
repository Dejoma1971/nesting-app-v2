...
export const NestingBoard: React.FC<NestingBoardProps> = ({
      ...
      const timer = setTimeout(() => {
        const doAutoSearch = async () => {
          ...
          try {
            ...
            if (response.status === 404) { alert(`Nenhuma peça encontrada para o pedido: ${initialSearchQuery}`); setIsSearching(false); return; }
          ...
          } catch(e) { console.error(e); alert("Erro ao buscar dados iniciais."); } finally { setIsSearching(false); }
        };
        doAutoSearch();
      }, 100);
  ...
  const handleDBSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/pecas/buscar?${params.toString()}`);
      if (response.status === 404) { alert("Nenhum pedido encontrado."); setIsSearching(false); return; }
      if (!response.ok) throw new Error("Erro ao buscar.");
      const data = await response.json();
      if (Array.isArray(data) && data.length > 0) {
        const dbParts: ImportedPart[] = data.map((item: any) => ({
        ...
        if (searchMode === 'replace') {
            if (nestingResult.length > 0) {
                if (!window.confirm("Isso limpará o arranjo atual da mesa. Deseja continuar?")) { setIsSearching(false); return; }
            }
            setParts(dbParts);
            resetNestingResult([]);
            resetProduction(); 
            ...
            setParts((prev) => {
                ...
                if (newUnique.length === 0) { alert("As peças desse pedido já estão na lista!"); return prev; }
    ...
    } catch (err) { console.error(err); alert("Erro de conexão."); } finally { setIsSearching(false); }
  };
  ...
  const handlePartsMove = useCallback((moves: { partId: string; dx: number; dy: number }[]) => {
      ...
      setNestingResult((prev) => {
        ...
        return prev.map((p) => { const move = moveMap.get(p.uuid); return move ? { ...p, x: p.x + move.dx, y: p.y + move.dy } : p; });
      });
  }, [setNestingResult]);
  ...
  const handleThumbnailContextMenu = (e: React.MouseEvent, partId: string) => {
    e.preventDefault(); e.stopPropagation(); setEditingPartId(partId);
  };
  ...
  const handleCalculate = useCallback(() => {
    ...
    if (nestingResult.length > 0) {
        if (!window.confirm("O cálculo automático irá REORGANIZAR TODA A MESA e apagar posicionamentos manuais.\n\nSe você está fazendo um 'mix' manual, CANCELE e arraste as peças manualmente.\n\nDeseja continuar e resetar a mesa?")) { return; }
    }
    setIsComputing(true);
    resetNestingResult([]);
  ...
  }, [displayedParts, quantities, gap, margin, binSize, strategy, iterations, rotationStep, direction, resetNestingResult, nestingResult.length]);
  ...
  const modalOverlayStyle: React.CSSProperties = { position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.6)", zIndex: 9999, display: "flex", justifyContent: "center", alignItems: "center" };
  const modalContentStyle: React.CSSProperties = { backgroundColor: theme.panelBg, padding: "25px", borderRadius: "8px", width: "350px", border: `1px solid ${theme.border}`, boxShadow: "0 4px 15px rgba(0,0,0,0.3)" };

  const renderProgressBar = (produced: number, total: number, color: string) => {
    const pct = Math.min(100, Math.round((produced / total) * 100));
    return ( <div style={{ width: "100%", background: theme.border, height: "6px", borderRadius: "3px", marginTop: "5px", overflow: "hidden" }}><div style={{ width: `${pct}%`, background: pct >= 100 ? "#28a745" : color, height: "100%" }}></div></div> );
  };

  return (
    <div style={containerStyle}>
      {isSearchModalOpen && (
        <div style={modalOverlayStyle} onClick={() => setIsSearchModalOpen(false)}>
          <div style={modalContentStyle} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: "15px", display: "flex", flexDirection: "column", gap: "10px", background: theme.inputBg, padding: "10px", borderRadius: "4px" }}>
                ...
                <div style={{display: "flex", gap: "15px"}}>
                    <label style={{ display: "flex", alignItems: "center", cursor: "pointer", fontSize: "13px", color: theme.text }}><input type="radio" name="searchMode" checked={searchMode === 'replace'} onChange={() => setSearchMode('replace')} style={{ marginRight: "5px" }} />Nova Mesa (Limpar)</label>
                    <label style={{ display: "flex", alignItems: "center", cursor: "pointer", fontSize: "13px", fontWeight: "bold", color: "#28a745" }}><input type="radio" name="searchMode" checked={searchMode === 'append'} onChange={() => setSearchMode('append')} style={{ marginRight: "5px" }} />Adicionar (Mix)</label>
                </div>
            </div>
      ...
      <div style={topBarStyle}>
        <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
          {onBack && (
            <button onClick={onBack} title="Voltar" style={{ background: "transparent", border: "none", color: theme.text, cursor: "pointer", fontSize: "24px", display: "flex", alignItems: "center", padding: 0 }}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg></button>
          )}
          <h2 style={{ margin: 0, fontSize: "18px", color: "#007bff", whiteSpace: "nowrap" }}>Planejamento de Corte</h2>
        </div>
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "10px" }}>
            <button onClick={() => setIsSearchModalOpen(true)} style={{ background: "#6f42c1", color: "white", border: "none", padding: "6px 12px", borderRadius: "4px", cursor: "pointer", fontWeight: "bold", display: "flex", alignItems: "center", gap: "5px", fontSize: "13px" }}>🔍 Buscar Pedido</button>
            <button style={{ background: isComputing ? "#666" : "#28a745", color: "white", border: "none", padding: "6px 12px", cursor: isComputing ? "wait" : "pointer", borderRadius: "4px", fontWeight: "bold", fontSize: "13px" }} onClick={handleCalculate} disabled={isComputing}>{isComputing ? "..." : "▶ Calcular"}</button>
            <button onClick={() => handleProductionDownload(nestingResult, currentBinIndex, displayedParts)} disabled={nestingResult.length === 0 || isSaving} style={{ background: lockedBins.includes(currentBinIndex) ? "#17a2b8" : "#007bff", color: "white", border: "none", padding: "6px 12px", cursor: (nestingResult.length === 0 || isSaving) ? "not-allowed" : "pointer", borderRadius: "4px", opacity: (nestingResult.length === 0 || isSaving) ? 0.5 : 1, fontSize: "13px", display: "flex", alignItems: "center", gap: "5px" }}>{isSaving ? "Salvando..." : lockedBins.includes(currentBinIndex) ? "⬇ Baixar Novamente" : "💾 Salvar DXF"}</button>
            <button onClick={handleClearTable} title="Limpar Mesa" style={{ background: "transparent", color: "#dc3545", border: `1px solid #dc3545`, padding: "5px 10px", borderRadius: "4px", cursor: "pointer", fontWeight: "bold", fontSize: "13px" }}>🗑️</button>
            ...
            {activeTab === "grid" && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: "15px", alignContent: "start" }}>
                {sortedParts.map((part) => {
                   ...
                   // Opacidade: Se concluída -> 0.6. Senão -> 1.
                   const opacity = isDoneVisual ? 0.6 : 1; 
                   
                   // LÓGICA DE BORDA "C" (Três Lados)
                   // Esquerda: Sempre cor do pedido
                   // Topo, Direita, Baixo: Verde (se na chapa), Azul (se selecionado), Padrão
                   const mainBorderColor = isOnCurrentSheet ? "#28a745" : (isSelected ? "#007bff" : theme.border);
                   const mainBorderWidth = isOnCurrentSheet ? "3px" : "1px";

                   const cardBorderStyle = {
                       borderLeft: `5px solid ${orderColor}`, // Fixo
                   ...
                   // Cursor e Drag: Bloqueia se concluído
                   ...
                   return (
                    <div 
                        ...
                        style={{ display: "flex", flexDirection: "column", alignItems: "center", position: "relative", opacity: opacity, cursor: cursorStyle }} 
                    ...
                    >
                        ...
                        {/* CARD DA PEÇA COM ESTILO DE BORDA DINÂMICO */}
                        <div style={{ 
                            width: "100%", aspectRatio: "1/1", background: theme.cardBg, 
                            borderRadius: "8px", marginBottom: "5px", padding: "10px", boxSizing: "border-box", 
                            display: "flex", alignItems: "center", justifyContent: "center", 
                            boxShadow: isSelected ? "0 0 5px rgba(0,123,255,0.5)" : "none",
                            ...cardBorderStyle // Aplica a borda calculada
                        }}>
                        ...
                        <div style={{ width: "100%", display: "flex", flexDirection: "column", fontSize: "12px" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                <div style={{display:'flex', flexDirection:'column', width: '65%'}}><span title={part.name} style={{ fontWeight: "bold", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{part.name}</span><span style={{fontSize: "10px", fontWeight: "bold", color: orderColor}}>Ped: {part.pedido || '-'}</span></div>
                            ...
                            <div style={{ fontSize: "10px", color: isDoneVisual ? "#28a745" : theme.label, display: "flex", justifyContent: "space-between" }}><span>{isDoneVisual ? "✅ CONCLUÍDO" : "Em Produção"}</span><span style={{ fontWeight: "bold" }}>Falta: {remainingVisual}</span></div>
                        </div>
                    </div>
                  );
                })}
              ...
              <div style={{ overflowX: "auto", transform: "rotateX(180deg)", borderBottom: `1px solid ${theme.border}` }}>
                <table style={{ width: "100%", borderCollapse: "collapse", borderSpacing: 0, minWidth: "600px", transform: "rotateX(180deg)" }}>
                  <thead style={{ background: theme.panelBg }}>
                    <tr><th style={thStyle}>#</th><th style={thStyle}>Peça</th><th style={thStyle}>Pedido</th><th style={thStyle}>Mat/Esp</th><th style={thStyle}>Dimensões</th><th style={thStyle}>Área</th><th style={thStyle}>Meta</th><th style={thStyle}>Status Produção</th></tr>
                  </thead>
                  <tbody>
                    {sortedParts.map((part, index) => {
                       ...
                       return (
                        <tr key={part.id} style={{ borderBottom: `1px solid ${theme.border}`, background: rowBg }}>
                            ...
                            <td style={{ ...tdStyle, textAlign: "center" }}><div style={{ padding: "4px 8px", borderRadius: "4px", background: isDoneVisual ? "rgba(40, 167, 69, 0.1)" : theme.hoverRow, border: `1px solid ${isDoneVisual ? "#28a745" : theme.border}`, color: isDoneVisual ? "#28a745" : theme.text, fontWeight: "bold", fontSize: "12px", display: "inline-block", minWidth: "60px" }}>{totalVisual} <span style={{fontSize: "10px", fontWeight: "normal", opacity: 0.7}}>de {qty}</span></div></td>
                            <td style={{ ...tdStyle, width: "120px" }}><div style={{ display: "flex", justifyContent: "space-between", fontSize: "10px", marginBottom: "2px" }}><span style={{fontWeight: 'bold'}}>Saldo:</span><span>{remainingVisual === 0 ? "✅" : `-${remainingVisual}`}</span></div>{renderProgressBar(totalVisual, qty, orderColor)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
